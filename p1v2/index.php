...
if($latlongcount >10){
  ...
  class mywaypoints implements JsonSerializable
  {
      protected $lat1;
      protected $long1;
      protected $temp1;
  ...
  $sql2 = "SELECT * FROM routes_data WHERE start='$start' AND end='$end'";
  $result = $conn->query($sql2);

  if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
         $dbdata=json_decode($row['data']);
         array_push($allinonearray,$dbdata);
        ...
        for($x =0; $x < $countoffinallatlong; $x++){
          ...
          $mywaypoints = new mywaypoints(array('lat' => round($finallatlong[$x][0],2), 'lng' => round($finallatlong[$x][1],2),'temp'=>round($weather_decode["main"]["temp"]-273.15,0),'place'=>$weather_decode["name"]));

           array_push($allinonearray, $mywaypoints);

      }
      $x=json_encode($allinonearray);
        $sql1 = "INSERT INTO routes_data (start, end, data,created_at)
        VALUES ('$start', '$end', '$x', SYSDATE())";
